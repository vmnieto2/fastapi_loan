...
@param_router.post('/params/get_type_document', tags=["Params"], dependencies=[Depends(JWTBearer())])
@http_decorator
def get_type_document(request: Request):
    response = Param().get_type_document()
    return response

@param_router.post('/params/get_type_user', tags=["Params"], dependencies=[Depends(JWTBearer())])
@http_decorator
